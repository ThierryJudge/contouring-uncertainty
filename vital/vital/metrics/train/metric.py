...
class DifferentiableDiceCoefficient(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            apply_activation: If ``True``, applies softmax or sigmoid to the input before computing the score.
                Otherwise, assumes the input is already normalized.
        """
        super().__init__()
    ...
    def forward(self, input: Tensor, target: Tensor) -> Tensor:
        ...
        Args:
            input: (N, C, H, W), Raw, unnormalized (or normalized if ``apply_activation == False``) scores for each
                class.

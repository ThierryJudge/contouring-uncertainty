...
from contour_uncertainty.models import resnet
from contour_uncertainty.models.deeplabv3.utils import _SimpleSegmentationModel, _load_weights
...
def _deeplabv3_resnet(
    ...
    if n_heads > 1:
        ...
        for _ in range(n_heads - 1):
            classifier.append(DeepLabHead(2048, num_classes))
        classifier = nn.ModuleList(classifier)

    return DeepLabV3(backbone, classifier, aux_classifier, features=features)
...
def deeplabv3_resnet50(
    ...
    Args:
        ...
        features: Add features from backbone to output.
    """
    if pretrained:
        aux_loss = True
    ...
    backbone = resnet.resnet50(pretrained=pretrained_backbone, replace_stride_with_dilation=[False, True, True],
                               **kwargs)
    model = _deeplabv3_resnet(backbone, num_classes, aux_loss, features=features, n_heads=n_heads)
...
def deeplabv3_mobilenet_v3_large(
    ...
    Args:
        ...
        pretrained_backbone (bool): If True, the backbone will be pre-trained.
    """
    assert not features, 'Not implemented'
    assert n_heads == 1, 'Not implemented'

    if pretrained:
        aux_loss = True
        pretrained_backbone = False

    backbone = mobilenetv3.mobilenet_v3_large(pretrained=pretrained_backbone, dilated=True, **kwargs)

...
from contour_uncertainty.models.nnUnet.layers import (
    ConvBlock,
    OutputBlock,
    ResidBlock,
    UpsampleBlock,
...
class ConfidenceNet(nn.Module):
    ...
    def __init__(self, output_size):
        ...
        self.model = nn.Sequential(
            nn.Conv2d(480, 128, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        assert not (ssn_rank != 0 and deep_supervision)
        if self.ssn_rank != 0:
            if ssn_rank == 1:
                self.ssn_blocks = [
                    OutputBlock(
                ...
                self.ssn_blocks = [
                    ...
                    OutputBlock(
                        in_channels=self.filters[0],
                        out_channels=self.num_classes * self.ssn_rank,
                        dim=self.dim,
                    )
                ]
            self.ssn_blocks = nn.ModuleList(self.ssn_blocks)
    ...
    def forward(
        ...
        if self.ssn_rank != 0:
            out = [out]
            for i in range(len(self.ssn_blocks)):
                out.append(self.ssn_blocks[i](upsample_out))
...
if __name__ == "__main__":
    ...
    # unet = UNet((1, 640, 512), (3,  640, 512), [ 640, 512], kernels, strides)
    unet = UNet((1, 256, 256), (3, 256, 256), [256, 256], kernels, strides,
                ssn_rank=5)
    # print(unet)

...
if __name__ == "__main__":
    ...
    ax[0].scatter(mu_p[:, 0], mu_p[:, 1], s=10, c='r')
    # # ax[0].scatter(gt[:, 0], gt[:, 1], s=10, c='b', label='Gt')
    mean_contour = contour_spline(mu_p.numpy())
    ax[0].plot(mean_contour[:, 0], mean_contour[:, 1], c='r', linewidth=1)
    ...
    for k in range(21):
        confidence_ellipse(mu_p[k, 0],mu_p[k, 1],cov_p[k],ax[0], edgecolor=colors[0], linewidth=1)
    ...
    for i in range(min(10, len(naive_contours))):
        ...
        ax[1].plot(sample[:, 0], sample[:, 1], linewidth=1)
    ...
    print(sampler.initial_points)
    print(sampler.points_order)

    # sampler.initial_points = [0, 10, 19]
    # sampler.points_order[0].append(20)
    # sampler.points_order[-1].remove(19)
    ...
    for k in range(0, 21):
        ax[0].text(mu_p[k, 0], mu_p[k, 1], f"{det[k]:.2f}", c='r')
    ...
    print(f'Generated {n} samples ({contours.shape}) in {total_n} sec. ({total_n / n} per sample)')
    ...
    for i in range(min(10, len(contours))):
        ...
        ax[2].plot(sample_contour[:, 0], sample_contour[:, 1], linewidth=1)
